import { APP_LOGO, COMPANY_NAME, COMPANY_DESCRIPTION, COMPANY_PHONE, COMPANY_EMAIL, COMPANY_LOCATION, COMPANY_SLOGAN, HERO_BACKGROUNDS, PRODUCTS, GALLERY_IMAGES, SOCIAL_LINKS } from "./data.js";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const init = () => {
        ...
        document.getElementById("hero-description").textContent = COMPANY_DESCRIPTION;
        ...
        const SOCIAL_ICONS = {
            facebook: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-facebook"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>',
    ...
    const renderHeroCarousel = () => {
        heroCarousel.innerHTML = HERO_BACKGROUNDS.map((bg, idx) => `
            <div class="absolute inset-0 transition-opacity duration-1000 ${idx === bgIndex ? 'opacity-100' : 'opacity-0'}" 
    ...
    const renderHeroIndicators = () => {
        heroCarouselIndicators.innerHTML = HERO_BACKGROUNDS.map((_, idx) => `
            <button class="w-3 h-3 rounded-full transition-all ${idx === bgIndex ? 'bg-primary w-8' : 'bg-white/50'}" data-index="${idx}"></button>
        `).join("");
    };
    ...
    const renderCategoryFilters = () => {
        ...
        categoryFilters.innerHTML = categories.map(cat => `
            <button class="px-4 py-2 rounded-full font-medium transition-all ${selectedCategory === cat ? 'bg-primary text-white' : 'bg-background text-foreground border border-primary/20 hover:border-primary'}" data-category="${cat}">${cat}</button>
        `).join("");
    };
    ...
    const renderProducts = () => {
        ...
        productsGrid.innerHTML = filteredProducts.map(product => {
            ...
            return `
                <div class="overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer bg-white rounded-lg" data-product-id="${product.id}">
                    <div class="relative h-64 bg-gray-200 overflow-hidden">
                        <img src="${currentImage}" alt="${product.name}" class="product-image w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                        <div class="absolute top-3 right-3 bg-black/70 text-white px-3 py-1 rounded-full text-sm font-semibold" style='color:#716767; font-weight:bold;'>${currentImageIndex + 1}/${productImages.length}</div>
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-sm text-foreground/70 mb-4">${product.description}</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-semibold text-primary bg-primary/10 px-3 py-1 rounded-full">${product.category}</span>
                            <button class="view-details bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-md" data-product-id="${product.id}">View Details</button>
                        </div>
                    </div>
                </div>
            `;
...
setInterval(() => {
    ...
    const renderGallery = () => {
        galleryGrid.innerHTML = GALLERY_IMAGES.map((img, idx) => `
            <div class="h-64 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow group">
                <img src="${img}" alt="Gallery ${idx + 1}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
            </div>
        `).join("");
    };
    ...
    const renderFooterProducts = () => {
        ...
        footerProducts.innerHTML = productLinks.map(link => `<li><a href="#" class="hover:underline">${link}</a></li>`).join("");
    };
    ...
    const handleProductInteraction = (e) => {
        ...
        if (e.target.classList.contains("prev-image")) {
            e.stopPropagation();
            imageIndices[productId] = (imageIndices[productId] || 0) === 0 ? productImages.length - 1 : (imageIndices[productId] || 0) - 1;
            renderProducts();
        } else if (e.target.classList.contains("next-image")) {
            e.stopPropagation();
            imageIndices[productId] = ((imageIndices[productId] || 0) + 1) % productImages.length;
            renderProducts();
        } else if (e.target.classList.contains("view-details") || e.target.closest('[data-product-id]')) {
            const currentImageIndex = imageIndices[product.id] || 0;
            selectedProduct = {
                ...product,
    ...
    const openModal = () => {
        ...
        modalBenefits.innerHTML = selectedProduct.benefits.map(b => `<li>${b}</li>`).join("");
...
// const mapUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(locationText)}`;

// const locationEl = document.getElementById("contact-location");
// locationEl.textContent = locationText;
// locationEl.href = mapUrl;
// locationEl.target = "_blank"; // open in new tab
// locationEl.rel = "noopener noreferrer";


// code to open the location in maps.
const locationText = COMPANY_LOCATION;
...
document.getElementById("contactForm").addEventListener("submit", function(e) {
    ...
    const phoneNumber = 9987493978; // e.g. 919876543210 (without +)
    ...
    *Message:* ${message}
    ...
    if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
        ...
        window.location.href = `whatsapp://send?phone=${phoneNumber}&text=${encodeURIComponent(text)}`;
    } else {
